...
2. 准备配置文件：
   ...
   cp .env.example .env
   ```
   并在 `config.local.js` 中配置您的 FastGPT 嵌入链接；后台权限由 Authelia 传入的 `remote-user` 控制，默认允许 `hekaixuan`。
3. 启动容器：
   ```bash
...
- **后台权限**：由 Authelia 的 `remote-user` 请求头控制，可通过环境变量 `ADMIN_REMOTE_USER` 调整允许访问的账号。
